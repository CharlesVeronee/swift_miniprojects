...
class Recipe: Identifiable {
    ...
    var unsectionedIngredients: [RecipeIngredient] {
        return ingredients.filter {$0.sectionLabel == nil}
            .sorted {$0.position < $1.position}
    }
    
    func ingredientsForSectionLabel (_ label: String) -> [RecipeIngredient] {
        return ingredients.filter {$0.sectionLabel == label}
            .sorted {$0.position < $1.position}
    }
    
    static func mealCourseFilter(course: MealCourse, recipes: [Recipe]) -> [Recipe] {
        return recipes.filter {recipe in
            recipe.mealCourse == course
        }
    }
    
    var instructionsOrdered: [Instruction] {
        return instructions.sorted {$0.position < $1.position}
    }
    
    init(name: String, details: String? = nil, credit: String? = nil, thumbnailUrl: URL? = nil, mealCourse: MealCourse, ingredients: [RecipeIngredient] = [], instructions: [Instruction] = [], sectionLabels: [String] = [], lastPreparedAt: Date? = nil, notes: String = "") {
        self.id = UUID()
        self.name = name
...
extension Recipe {
    ...
    var dataForForm: FormData {
        FormData(
            ...
            notes: notes,
            previouslyPrepared: lastPreparedAt != nil
        )
    }
    ...
    static func create(from formData: FormData, context: ModelContext) {
        let recipe = Recipe(name: formData.name, details: formData.details, credit: formData.credit, mealCourse: formData.mealCourse, notes: formData.notes)
    ...
    static func update(_ recipe: Recipe, from formData: FormData) {
        ...
        recipe.lastPreparedAt = formData.previouslyPrepared ? formData.lastPreparedAt : nil
    }
  }

class RecipeIngredient: Identifiable, Codable {
    var id: UUID
    ...
    init(ingredient: Ingredient, quantity: Double? = nil, unit: String? = nil, note: String? = nil, position: Int = 999999, sectionLabel: String? = nil) {
        self.id = UUID()
    ...
    func ingredientDisplay(_ recipeIngredient: RecipeIngredient, scaleFactor: Double) -> String {
        var returnString = ""
        if let quantity = recipeIngredient.quantity {
            let scaledQuantity = quantity * scaleFactor
            returnString.append(String(scaledQuantity) + " ")
        }
        ...
        returnString.append(String(recipeIngredient.ingredient.name))
        return returnString
    }
}
...
class Instruction: Identifiable, Codable {
    var id: UUID
    var position: Int
    var instructionText: String
    
    init(position: Int = 999999, instructionText: String) {
        self.id = UUID()
        self.position = position
...
extension Recipe {
    ...
    enum ItalianWine: String, Codable {
        case barolo
        case brunello
        case taurasi
    }
...
extension Recipe {
  static let previewData: [Recipe] = [
    ...
    Recipe(
      ...
      details: "Also known as a Neopolitan ragu or sometimes 'pasta all Genovese', though it is from Campania and not Liguria, hence the pairing with the Taurasi.",
