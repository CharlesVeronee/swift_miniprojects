...
struct Recipe: Identifiable {
    ...
    init(name: String, details: String? = nil, credit: String? = nil, thumbnailUrl: URL? = nil, cuisine: Cuisine, mealCourse: MealCourse, ingredients: [RecipeIngredient] = [], instructions: [Instruction] = [], sectionLabels: [String]? = nil) {
        self.id = UUID()
        self.name = name
...
func printIngredients(recipe: Recipe) {
    recipe.unsectionedIngredients.forEach {ingredient in
    ...
    if let sectionLabels = recipe.sectionLabels {
        sectionLabels.forEach {label in
            ...
            let withSection = recipe.ingredientsForSectionLabel(label)
            for ingredient in withSection {
                print(ingredient.ingredient.name)
            }
...
extension Recipe {
  static let previewData: [Recipe] = [
    ...
    Recipe(
      ...
      cuisine: .italian(winePairing: .taurasi),
      mealCourse: .main,
      ingredients: [
...
let mainDish = Recipe.mealCourseFilter(course: .main, recipes:
Recipe.previewData).first
if let finalAnswer = mainDish {
printIngredients(recipe: finalAnswer)
} else {
print("Keep Trying!")
}
