...
enum Cuisine {
    ...
    var cuisineString: String {
            ...
            case .american:
                return "American"
            case let .chinese(region):
                return "Chinese - \(region)"
            case .japanese:
                return "Japanese"
            case let .italian(winePairing):
                return "Italian, pair with a \(winePairing.rawValue.lowercased())"
            }
        }
}

enum ItalianWine: String {
    case barolo
    case brunello
    case taurasi
}
